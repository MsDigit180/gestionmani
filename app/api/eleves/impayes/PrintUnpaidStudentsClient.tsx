...
export default function PrintUnpaidStudentsClient({
  initialEleves,
}: {
  ...
  // Filtrage des élèves qui N'ONT PAS réglé pour le mois et l'année sélectionnés
  const filteredEleves = initialEleves.filter((eleve) => {
    ...
    // Inversion de la condition : !aPayeCeMois pour isoler les impayés
    return !aPayeCeMois && matchNiveau && matchEcole && matchQuartier;
  });

  // Calcul du montant total restant à recouvrir (somme des impayés)
  const totalImpayes = filteredEleves.reduce((sum, eleve) => {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-100 flex items-center gap-2">
              ...
              Impression des Élèves en Retard de Paiement (Impayés)
            </h1>
          </div>
          <button
            onClick={handlePrint}
            disabled={filteredEleves.length === 0}
            className="flex items-center gap-2 px-5 py-2.5 bg-amber-600 hover:bg-amber-500 disabled:bg-slate-700 text-white rounded-lg text-sm font-semibold transition cursor-pointer"
          >
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-amber-400 font-bold"
            >
              {MOIS_LISTE.map((m) => (
      ...
      <div className="printable-area bg-slate-800 border border-slate-700 rounded-xl overflow-hidden print:bg-white print:text-black print:border-none p-0 print:p-0">
        ...
        <div className="print-header mb-6 text-center">
          <h1 className="text-xl font-bold uppercase tracking-wide text-black border-b-2 border-black pb-2 mb-1">
            Liste des élèves n'ayant pas encore réglé leur paiement pour le mois de {selectedMois} {selectedAnnee}
          </h1>
          <p className="text-xs text-gray-600 font-medium">
            Imprimé le {new Date().toLocaleDateString('fr-FR')} - Total : {filteredEleves.length} élève(s) en retard
          </p>
        </div>

        {filteredEleves.length === 0 ? (
          <div className="p-8 text-center text-slate-400 print:text-black">
            Aucun impayé trouvé pour <strong className="text-amber-400 print:text-black">{selectedMois} {selectedAnnee}</strong>. Tous les élèves sont à jour !
          </div>
        ) : (
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-slate-900/50 text-slate-400 uppercase text-xs border-b border-slate-700 print:bg-gray-200 print:text-black">
              <tr>
                ...
                <th className="px-6 py-3.5 font-bold text-right">Frais Dûs</th>
            ...
            <tbody className="divide-y divide-slate-700/50 print:divide-black">
              {filteredEleves.map((eleve, index) => (
                <tr key={eleve.id}>
                  ...
                  <td className="px-6 py-4 text-right font-mono text-amber-400 print:text-black">
                    {Number(eleve.frais_encadrement || 0).toLocaleString('fr-FR')} FCFA
                  </td>
                  <td className="px-6 py-4 text-center font-bold text-rose-500 print:text-black">
                    NON PAYÉ
                  </td>
                </tr>
              ))}
            </tbody>
            {/* Ligne du Total Général des Impayés */}
            <tfoot className="bg-slate-900/80 border-t-2 border-slate-600 print:bg-gray-100 print:border-black font-bold">
              <tr>
                <td colSpan={6} className="px-6 py-3.5 text-right uppercase tracking-wider text-xs text-slate-300 print:text-black">
                  TOTAL IMPAYÉS À RECOUVRIR :
                </td>
                <td className="px-6 py-3.5 text-right font-mono text-rose-400 print:text-black text-base">
